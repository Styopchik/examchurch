...
use App\Notifications\NewFormSurveyDetails;
...
class FormController extends Controller
{
    public function index(FormsDataTable $dataTable)
    {
        if (\Auth::user()->can('manage-form')) {
            ...
            return $dataTable->render('form.index', compact('categories', 'roles'));
        } else {
    ...
    {
        $formTemplates = FormTemplate::where('created_by', \Auth::user()->id)->whereNotNull('json')->where('status', 1)->get();
    ...
    public function create()
    {
        if (\Auth::user()->can('create-form')) {
            $users = User::where('id', '!=', 1)->where('created_by', Auth::user()->admin_id)->pluck('name', 'id');
            $roles = Role::where('name', '!=', 'Super Admin')->where('created_by', Auth::user()->admin_id)->orwhere('name', Auth::user()->type)->pluck('name', 'id');
            $category = FormCategory::where('status', 1)->pluck('name', 'id');
            return view('form.create', compact('roles', 'users', 'category'));
        } else {
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('create-form')) {
            ...
            $fileName = '';
            $request->validate([
                'form_logo'     => 'required|image|mimes:jpeg,jpg,png',
            ]);
            ...
            if (Auth::user()->type == 'Admin') {
                ...
                $forms      = Form::where('created_by', \Auth::user()->admin_id)->count();
                $plan       = Plan::find($user->plan_id);
                $userSchema = User::where('type', 'Admin')->first();
                if ($forms < $plan->max_form) {
                    ...
                    $form->created_by           = Auth::user()->admin_id;
                    ...
                    if (UtilityFacades::getsettings('email_setting_enable') == 'on') {
                        $notify = NotificationsSetting::where('title', '=', 'Form Create')->first();
                        if (isset($notify)) {
                            if ($notify->notify == '1') {
                    ...
                    return redirect()->route('forms.index')->with('failed', __('Your form limit is over, please upgrade plan.'));
                }
            } else {
                $user       = Auth::user()->created_by;
                $createdBy  = User::find($user);
                $forms      = Form::where('created_by', $user)->count();
                $plan       = Plan::find($createdBy->plan_id);
                if ($forms < $plan->max_form) {
                    if ($request->set_end_date == 'on') {
                        $setEndDate = 1;
                    } else {
    ...
    public function edit($id)
    {
        ...
        if (\Auth::user()->can('edit-form') && $user->type == $user->roles->first()->name) {
            $form               = Form::find($id);
            ...
            $formRoles          = Role::where('id', '!=', 1)->pluck('name', 'id');
            $formUser           = $form->assignedusers->pluck('id')->toArray();
            $formUsers          = User::where('id', '!=', 1)->where('created_by', Auth::user()->admin_id)->pluck('name', 'id');
            $categories         = FormCategory::where('status', 1)->pluck('name', 'id');

            return view('form.edit', compact('form', 'roles', 'formUser', 'formUsers', 'formRole', 'formRoles', 'next', 'previous', 'categories'));
        } else {
            if (\Auth::user()->can('edit-form') && $formAllowedEdit > 0) {
                ...
                $formRole       = $form->assignedroles->pluck('id')->toArray();
                $formRoles      = Role::pluck('name', 'id');
                $formUser       = $form->assignedusers->pluck('id')->toArray();
                $formUsers      = User::where('id', '!=', 1)->pluck('name', 'id');
                $categories     = FormCategory::where('status', 1)->pluck('name', 'id');
    ...
    public function fill($id)
    {
        if (\Auth::user()->can('fill-form')) {
            ...
            if ($form) {
                ...
                return view('form.fill', compact('form', 'formValue', 'plans', 'array'));
            } else {
                return redirect()->back()->with('failed', __('Form not found.'));
            }
        } else {
    ...
    public function publicFill($id)
    {
        ...
        if ($id) {
            $form                   = Form::find($id);
            $bacK_image             = $form->background_image;
            ...
            if ($form) {
                ...
                $array              = $form->getFormArray();
                $backgroundImage    = Storage::url($form->background_image);
                return view('form.public-fill', compact('form', 'formValue', 'array', 'backgroundImage'));
            } else {
                return redirect()->back()->with('failed', __('Form not found.'));
            }
        } else {
            ...
            if ($form->limit > $formValue || $request->form_value_id != null) {
                ...
                if ($form) {
                                                    ...
                                                    foreach ($files as $file) {
                                                        ...
                                                        if ($check) {
                                                            if ($extension == 'csv') {
                                                                ...
                                                                $file_name = $file->store('form-values/' . $form->id);
                                                                if (!file_exists($path)) {
                    ...
                    if ($request->form_value_id) {
                        ...
                        if ($form->payment_status == 1) {
                            if ($form->payment_type == 'stripe') {
                                ...
                                if ($charge) {
                                    ...
                                    $orderData['currency_symbol'] = $form->currency_symbol;
                                    $orderData['currency_name']   = $form->currency_name;
                                    $orderData['amount']          = $form->amount;
                                ...
                                $orderData['currency_name']    = $form->currency_name;
                                $orderData['amount']           = $form->amount;
                                $orderData['payment_type']     = 'offlinepayment';
                                ...
                                if ($request->file('transfer_slip')) {
                                    ...
                                    $orderData['transfer_slip'] = $filename;
                                }
                        ...
                        $orderData['form_id']                   = $form->id;
                        $orderData['user_id']                   = $userId;
                        $orderData['json']                      = json_encode($array);
                        $orderData['form_edit_lock_status']     = $form->form_fill_edit_lock;

                        $formValue          = FormValue::create($orderData);
                    }
                    ...
                    if (isset($notificationsSetting)) {
                        if ($notificationsSetting->notify == '1') {
                            $user->notify(new NewFormSurveyDetails($form));
    ...
    public function formStatus($id, Request $request)
    {
        $form = Form::find($id);
    ...
    public function gridView($slug = '')
    {
        ...
        if ($usr->type == 'Super Admin') {
            ...
            $forms          = Form::where('created_by', $usr->admin_id);
        } else {
            $forms          = Form::where(function ($query) use ($roleId, $userId) {
                $query->whereIn('id', function ($query1) use ($roleId) {
                    $query1->select('form_id')->from('assign_forms_roles')->where('role_id', $roleId);
    ...
    public function formPaymentIntegration(Request $request, $id)
    {
        ...
        // Form's offline payment integartion
        if (UtilityFacades::getsettings('offlinepaymentsetting') == 'on') {
            $paymentType['offlinepayment'] = 'Form offline payment';
        }
    ...
    public function formIntegrationStore(Request $request, $id)
    {
        ...
        $mailgunData          = [];
        $mailgunFiledText     = [];
        if ($request->get('mailgun_email') && $request->get('mailgun_domain') && $request->get('mailgun_secret') && $request->get('mailgun_mail_from_address')) {
            foreach ($request->get('mailgun_email') as $mailgunKey => $mailgunvalue) {
                ...
                $mailgunData[$mailgunKey]['mailgun_mail_from_address']      = $request->get('mailgun_mail_from_address')[$mailgunKey];
        ...
        FormIntegrationSetting::updateorcreate(
            ...
            ['status' => ($request->get('mailgun_email') && $request->get('mailgun_domain') && $request->get('mailgun_secret') && $request->get('mailgun_mail_from_address')) ? 1 : 0, 'field_json' => json_encode($mailgunFiledText), 'json' => $mailgunJsonData]
        );
