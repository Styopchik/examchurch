...
class PlanDataTable extends DataTable
{
        ...
        return datatables()
            ...
            ->editColumn('active_status', function (plan $plan) {
                if ($plan->active_status == 1) {
                    ...
                    <input class="form-check-input chnageStatus" checked type="checkbox" role="switch" id="' . $plan->id . '" data-url="' . route('plan.status', $plan->id) . '">
                </div>';
                } else {
                    return '<div class="form-check form-switch">
                    <input class="form-check-input chnageStatus" type="checkbox" role="switch" id="' . $plan->id . '" data-url="' . route('plan.status', $plan->id) . '">
                </div>';
                }
            })
            ...
            ->editColumn('price', function (plan $plan) {
                return UtilityFacades::amountFormat($plan->price);
            })
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('max_roles')->title(__('Max Roles')),
            Column::make('max_form')->title(__('Max Forms')),
            Column::make('max_booking')->title(__('Max Bookings')),
            Column::make('max_documents')->title(__('Max Documents')),
            Column::make('max_polls')->title(__('Max Polls')),
