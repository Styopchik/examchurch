...
class FormTemplateDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ...
            ->editColumn('created_at', function ($request) {
            ...
            })->editColumn('image', function (FormTemplate $row) {
                if ($row->image) {
                ...
                return $image;
            })->editColumn('status', function (FormTemplate $row) {
                if ($row->status == 1) {
                    $status = '<div class="form-check form-switch">
                                            <input class="form-check-input chnageStatus" checked type="checkbox" role="switch" id="' . $row->id . '" data-url="' . route('formTemplate.status', $row->id) . '">
                                        </div>';
                } else {
                    $status =  '<div class="form-check form-switch">
                                            <input class="form-check-input chnageStatus" type="checkbox" role="switch" id="' . $row->id . '" data-url="' . route('formTemplate.status', $row->id) . '">
                                        </div>';
                }
                return $status;
            })
            ->addColumn('action', function (FormTemplate $row) {
                return view('form-template.action', compact('row'));
            })
            ...
            ->parameters([
                ...
                'buttons'   => [
                    ['extend' => 'create', 'className' => 'btn btn-light-primary no-corner me-1 add_module', 'action' => " function ( e, dt, node, config ) {
                                window.location = '" . route('form-template.create') . "';
                           }"],
                    [
