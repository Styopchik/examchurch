@extends('layouts.main')
@section('title', 'Permissions')
@section('breadcrumb')
    <div class="col-md-12">
        <div class="page-header-title">
            <h4 class="m-b-10">{{ __('Permissions') }}</h4>
        </div>
        <ul class="breadcrumb">
            <li class="breadcrumb-item">{!! Html::link(route('home'), __('Dashboard'), ['']) !!}</li>
            <li class="breadcrumb-item">{!! Html::link(route('roles.index'), __('Roles'), ['']) !!}</li>
            <li class="breadcrumb-item">{{ __('Permissions') }}</li>
        </ul>
    </div>
@endsection
@section('content')
    <div class="row">
        <div class="section-body">
            <div class="row">
                <div class="mx-auto col-md-12">
                    {!! Form::open(['route' => ['roles.permit', $role->id], 'method' => 'POST']) !!}
                    <div class="card">
                        <div class="card-header">
                            <h5>{{ __('All Permissions') }}</h5>
                        </div>
                        <div class="card-body responive-body">
                            <table class="table table-flush permission-table">
                                <thead class="thead-light">
                                    <tr>
                                        <th>{{ __('Module') }}</th>
                                        <th>{{ __('Permissions') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($allModules as $module)
                                        @if ($module != 'module')
                                            <tr>
                                                <td> {{ ucfirst($module) }}</td>
                                                <td>
                                                    <div class="row">
                                                        @php
                                                            $defaultPermissions = ['manage', 'create', 'edit', 'delete', 'view', 'impersonate', 'fill', 'design', 'show', 'download', 'duplicate', 'theme-setting', 'integration', 'payment', 'vote', 'result', 'phone-verified', 'email-verified', 'plan-upgrade', 'document-generate','access-all'];
                                                        @endphp
                                                        @foreach ($defaultPermissions as $permission)
                                                            @if (in_array($permission . '-' . $module, $allPermissions))
                                                                @php($key = array_search($permission . '-' . $module, $allPermissions))
                                                                <div class="col-3 form-check custom-control custom-checkbox">
                                                                    {{ Form::checkbox('permissions[]', $key, in_array($permission . '-' . $module, $permissions), ['class' => 'form-check-input', 'id' => 'permission_' . $key]) }}
                                                                    {{ Form::label('permission_' . $key, ucfirst($permission), ['class' => 'form-check-label']) }}
                                                                </div>
                                                            @endif
                                                        @endforeach
                                                    </div>
                                                </td>
                                            </tr>
                                        @endif
                                    @endforeach
                            </table>
                        </div>
                        <div class="card-footer">
                            <div class="float-end">
                                <a href="{{ route('roles.index') }}" class="btn btn-secondary">{{ __('Cancel') }}</a>
                                {{ Form::button(__('Save'), ['type' => 'submit', 'class' => 'btn btn-primary']) }}
                            </div>
                        </div>
                    </div>
                    {!! Form::close() !!}
                </div>
            </div>
        </div>
    </div>
@endsection
