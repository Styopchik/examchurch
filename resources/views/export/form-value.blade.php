        ...
        @foreach ($formValues as $key => $formValue)
            ...
            <tr>
                @foreach (json_decode($formValue->json) as $jsons)
                            ...
                            @if (isset($json->value))
                                ...
                                @elseif ($json->type == 'SignaturePad')
                                    ...
                                    @else
                                        <td>null</td>
                                    @endif
                                @elseif ($json->type == 'repeater')
                                    @php
                                        $imageChoiceJsons = json_decode($json->value);
                                    @endphp
                                    @if ($imageChoiceJsons)
                                        @foreach ($imageChoiceJsons as $imageChoiceJson)
                                            @if (isset($imageChoiceJson->selected) && $imageChoiceJson->selected == 1)
                                                <td>
                                                    {!! Html::link(Storage::path($imageChoiceJson->image), $imageChoiceJson->label, []) !!}
                                                </td>
                                            @endif
                                        @endforeach
                                    @else
                    ...
                    @endif
                    <td>{{ isset($formValue->created_at) ? Utility::dateTimeFormat($formValue->created_at) : 'null' }}
                    </td> --}}
                @endforeach
            </tr>
        @endforeach
